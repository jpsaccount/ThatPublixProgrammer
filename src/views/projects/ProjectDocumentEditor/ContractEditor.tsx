...
import { Contract, ContractLineItem } from "@/sdk/entities/project/ProposalLineItem";
...
interface Props {
  contract: Contract;
  onContractSaved: (proposal: Contract) => any;

  workingPhases: WorkingPhase[];
}

const ContractEditor = ({ contract, workingPhases, onContractSaved }: Props) => {
  const [title, setTitle] = useState(contract?.Title);
  const [notes, setNotes] = useState(contract?.Notes);
  const [lineItems, setLineItems] = useState(contract?.Phases);

  const updatePhases = (id: string, newProps: Partial<ContractLineItem>) => {
    ...
    <div className="min-w-[80dvw]">
      ...
      <div className="grid grid-flow-col">
        ...
        <PolButton
          ...
          onClick={() => setLineItems((prevItems) => [...prevItems, new ContractLineItem()])}
        >
          <div className="flex">
      ...
      <div className="flex w-full items-center justify-between">
        <PolButton
          onClick={() =>
            onContractSaved && onContractSaved({ ...contract, Title: title, Notes: notes, Phases: lineItems })
          }
